...
const Home = (props) => {
  const { onClick, isPopupActive, homeData } = props;

  const { title, frames } = homeData.home;
    ...
    return (
      <div key={item.id} id={item.id} className="frame">
        <div className="frame__img">
          ...
          {index == 0 ? (
            <>
              <span className="frame__text frame__text--spain" aria-hidden>
                {item.spain}
              </span>
              <span className="frame__text frame__text--zealand" aria-hidden>
                {item.newZealand}
              </span>
            </>
          ) : null}
