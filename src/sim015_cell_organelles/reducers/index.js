...
const shuffleItems = data.draggableData.draggables;
const shuffleData = shuffleItems.sort(() => Math.random() - 0.5)
...
/* Screen 3 */
 ...
 const draggableItems = (state = [...shuffleData], action) => {
 ...
 const dropedItemList = (state = { plantDragList: [], animalDragList: [] }, action) => {
  switch (action.type) {
    case actions.ADD_PLANT_DROPPABLE:
 ...
 const forceAnimalHighlight = (state = false, action) => {
  switch (action.type) {
 ...
 const forcePlantHighlight = (state = false, action) => {
  switch (action.type) {
