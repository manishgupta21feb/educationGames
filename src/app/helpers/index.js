const SIM_LANGUAGES = ["en", "es"];
const DEFAULT_SIM_LANGUAGE = SIM_LANGUAGES[0];
...
export const getSimLanguage = () => {
  let url = window.location.href;
  let data = {};
  let hashes = url.split("?")[1];
  if (hashes == undefined) {
    return DEFAULT_SIM_LANGUAGE;
  }
  ...
  let lang = DEFAULT_SIM_LANGUAGE;
  if (data.lang && SIM_LANGUAGES.indexOf(data.lang) >= 0) {
    lang = data.lang;
  }
  return lang;
};
